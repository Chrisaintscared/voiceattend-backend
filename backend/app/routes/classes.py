...
def create_class(body: CreateClassRequest, user=Depends(get_current_user)):
    ...
    try:
        cur = conn.cursor()
        cur.execute("INSERT INTO classes (name, code, teacher_id) VALUES (%s, %s, %s) RETURNING id, name, code", 
...
def join_class(body: JoinClassRequest, user=Depends(get_current_user)):
    ...
    try:
        ...
        cls = cur.fetchone()
        if not cls: raise HTTPException(status_code=404, detail="Invalid code")
        cur.execute("INSERT INTO class_members (class_id, student_id) VALUES (%s, %s)", (cls[0], user['id']))
        conn.commit()
...
def my_classes(user=Depends(get_current_user)):
    ...
    try:
        cur = conn.cursor()
        if user['role'] == 'teacher':
            cur.execute("SELECT id, name, code FROM classes WHERE teacher_id = %s", (user['id'],))
        else:
            cur.execute("""SELECT c.id, c.name, c.code FROM classes c 
                        JOIN class_members cm ON cm.class_id = c.id WHERE cm.student_id = %s""", (user['id'],))
        rows = cur.fetchall()
...
def get_attendance(class_id: int, user=Depends(get_current_user)):
    ...
    try:
        cur = conn.cursor()
        cur.execute("SELECT user_name, timestamp FROM attendance_logs WHERE class_id = %s", (class_id,))
        rows = cur.fetchall()
